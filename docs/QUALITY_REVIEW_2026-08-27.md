...
3. **P1 - 本地书库重复索引丢失**
   - 问题：索引正在进行时的新索引请求会被直接忽略，用户连续授权目录时可能要到下次手动刷新或重启才看到新书。
   - 处理：改为合并请求的单消费者队列；索引期间的后续请求会在当前批次后执行一次。

4. **P1 - 大本地书库的重排和内存压力**
   - 问题：每本书元数据完成时都对整个列表重排，封面原始字节也会无上限驻留内存。
   - 处理：渐进补齐阶段只原位替换书卡，整批完成后只排序一次；封面内存缓存改为 8 MiB LRU，磁盘缩略图仍保留。
...
1. **P1 - 建立可量化的性能基线**
   - 新建 Macrobenchmark/Baseline Profile 模块，测量冷启动、首帧、发现页滚动、本地 100/500/2,000 本导入与二次打开。
   - 性能回归应使用 Release-like 构建和固定 AVD，不再用 Debug 模拟器的主观卡顿作为唯一依据。

2. **P1 - 本地索引从 SharedPreferences 迁移到 SQLite/Room**
   - 当前上限是 2,000 本，URI 和每本元数据以 SharedPreferences 键值保存，超过上限会被静默截断。
   - 下一版应用可查询索引表、唯一 URI 索引和批量事务，并对超限/不可读文件显示可见结果。
...
并验证：APK 签名、覆盖安装保留数据、两站分别登录和阅读、断网阅读、本地大书库二次打开、下载/导出、通知权限拒绝、深色模式和大字体。

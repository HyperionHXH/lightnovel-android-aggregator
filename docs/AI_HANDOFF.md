...
- 在线章节离线保存、离线书架、EPUB 3/TXT 导出；默认写入应用专用 `offline_library/exports`，SAF 目录写入 `Mixn/exports`，不再支持本地 EPUB 导入/本地阅读器。
...
Android 默认版本：`versionName 1.16.0`、`versionCode 25`；桌面和 jpackage 默认版本也是 `1.16.0`。tag Release 工作流会从 tag 注入版本。当前正式 Release 为 `v1.16.0`（本轮提交后创建）。
...
- 本轮新增 EPUB/TXT 导出：离线书籍卡片分别提供两个导出按钮，优先写入配置的下载目录；EPUB 使用现有封面/插图资源，TXT 使用 UTF-8 纯文本并以 `[插图]` 占位。滚动阅读插图增加稳定高度约束，放大预览改为长按显示操作栏、空白处直接退出。

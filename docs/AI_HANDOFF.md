...
- 文字样式提供系统默认、无衬线、衬线、等宽、手写体、紧凑无衬线和圆体；细黑、中黑、特黑已移除，旧版本值会回退到衬线字体。独立字体预览页可用统一样例比较系统字体和已安装下载字体；可按需下载思源宋体、思源黑体、霞鹜文楷及等宽版本，下载文件固定提交并做 SHA-256 校验，已安装字体不会重复下载。
...
Android 默认版本：`versionName 1.8.0`、`versionCode 14`；桌面和 jpackage 默认版本也是 `1.8.0`。tag Release 工作流会从 tag 注入版本。当前正式 Release 为 `v1.8.0`（本轮提交并推送后创建）。
...
- `v1.8.0` 本轮已在本机 `noval_api35` AVD 验证新手引导 4 步、字体预览页标题/系统字体/可下载字体入口和已删除字重；完整 Debug 构建、单元测试、Lint、桌面测试通过。在线冒烟测试需要预置真实双源登录态，未登录时不能作为本轮 UI 回归依据。

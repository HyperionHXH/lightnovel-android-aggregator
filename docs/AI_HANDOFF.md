...
- `v1.9.1` 为四种可下载中文字体内置了仅覆盖统一预览样例的真实字形子集（合计约 93 KB），完整字体仍按需下载；字体页优先展示中文阅读字体，设备字体明确提示 Android 的中文回退可能使多个选项看起来相同。设置页进一步拆分界面、文字排版、阅读交互和设备行为卡片，并提高边界与分隔线对比度。
...
Android 默认版本：`versionName 1.9.1`、`versionCode 16`；桌面和 jpackage 默认版本也是 `1.9.1`。tag Release 工作流会从 tag 注入版本。当前正式 Release 为 `v1.9.1`（本轮提交并推送后创建）。
...
- `v1.9.1` 本轮已在本机 `noval_api35` AVD 截图验证高对比设置分组，以及思源宋体、思源黑体、霞鹜文楷使用同一中英标点样例时的真实中文字形差异；完整 Debug 构建、单元测试、Lint、桌面测试及定向 Android 测试通过。在线冒烟测试需要预置真实双源登录态，未登录时不能作为本轮 UI 回归依据。

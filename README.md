<p align="center">
  <img src="docs/app-icon.svg" width="112" alt="诺阅图标">
</p>
...
- 本地书库支持通过系统文件选择器授权文件夹后自动读取 EPUB、TXT、HTML 和 FB2，也保留单独导入文件入口；EPUB 索引只读取元数据，正文按需加载，并显示书名、作者和封面。
- 设置中的“下载目录”支持授权一个文件夹保存离线书库；应用会在其中创建自己的诺阅数据子目录，不申请全盘存储权限。
